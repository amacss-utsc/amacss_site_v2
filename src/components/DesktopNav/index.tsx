...
export const DesktopSidebar: FC<Props> = ({ events, tags }) => {
    ...
    return (
      <nav className="min-w-[242px] hidden lg:flex lg:flex-col bg-gray-80 relative">
        ...
        <div className="flex-1 overflow-hidden relative px-3">
          ...
          {e.length === 0 ? (
            <p className="text-center text-gray-02 w-full mt-10">
              No events to show :(
            </p>
          ) : (
            <Splide
                  ...
                  <SplideSlide key={j}>
                    <article className="w-full max-w-[218px] max-h-[264px] min-h-[264px] bg-white rounded-b-[12px] flex flex-col relative overflow-hidden mb-11 ticker-tile">
                      ...
                      <div className="flex flex-col h-full px-2.5 py-1.5">
                        ...
                        <p className="text-[8px] line-clamp-1 text-ellipsis">
                          {ev.previewText}
                        </p>
      ...
      <nav className="min-w-[242px] hidden lg:flex lg:items-center lg:flex-col bg-gray-80 pt-[18.5px]">
        <Link href="/">
          <Logo
            ...
            width={200}
            height={30.25}
            className="min-h-[31px] mb-9"
          />
        </Link>

        <div className="w-full text-gray-10 relative p-6 pt-4 overflow-scroll">
              ...
              return (
                <div
                  ...
                  onClick={(e) => {
                    ...
                    else setTagsIndices([...tagsIndices, j])
                  }}
                >
                  <div
                    className={cn(
                      tagsIndices.includes(j)
                        ? "bg-blue-20 border-blue-20"
                        : "border-gray-10",
                  ...
                  <p
                    className={cn(
                      "select-none text-lg font-normal normal-case",
                      tagsIndices.includes(j) ? "text-gray-02" : "text-gray-10",
                    )}
                  >
                    {i}
                  </p>
          ...
          <Collapsible title={"Date Range"}>
            <div className="flex flex-col normal-case">
              ...
              <DatePicker
                className="bg-gray-80 hover:bg-gray-80 w-full"
                fDate={startDate || new Date()}
                fsetDateAction={setStartDate}
              />
              <label>End Date</label>
              <DatePicker
                className="bg-gray-80 hover:bg-gray-80 w-full"
                fDate={endDate || new Date()}
                fsetDateAction={setEndDate}
              />
            </div>
          </Collapsible>
        </div>
  ...
  else if (pathname.startsWith("/courses/")) return <nav></nav>
}
      ...
      {user == null ? (
        ...
        >
          <button
            className={cn(
              NavLink,
              "font-bold cursor-pointer w-fit uppercase h-full flex items-center justify-center group",
            )}
          >
