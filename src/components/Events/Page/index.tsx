...
export const EventsPage: FC<EventsPageProps> = ({ events, tags }) => {
  ...
  const { setFocusedEvent, setFilterOpen, tags: cTags, setTags, tagsIndices, startDate, endDate } = useStateContext();
      ...
      <section className="lg:grid lg:grid-cols-[repeat(auto-fit,_minmax(300px,_1fr))] lg:gap-10 overflow-y-scroll">
        {filteredEvents.length === 0 ? (
          ...
          filteredEvents.map((ev, j) => {
            ...
            const rT = typeof ev.ribbonTag !== "number" && ev.ribbonTag?.ribbonTag ? ev.ribbonTag?.ribbonTag : "";
            return (
              <article
                key={j}
              ...
              >
                ...
                <div className={cn(RibbonStyle, "bg-white opacity-20 h-[33px] top-[40.5px] left-[81px] w-[300px]")}>
                  {" "}
                </div>
                <Image src={url} alt={alt} width={width} height={height} className="w-full h-[189px] object-cover" />
                <div className="flex flex-col h-full px-2.5 py-1.5">
                  <h2 className="font-bold text-2xl text-gray-90">{ev.title}</h2>
                  ...
                  <div className="w-full flex items-end justify-between mt-auto uppercase ">
                    ...
                    <button className="text-blue-20 text-sm font-semibold flex items-center uppercase" onClick={() => setFocusedEvent(ev)}>
                      Learn more
