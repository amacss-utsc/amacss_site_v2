...
export default async function DashboardPage() {
  ...
  const getUrl = (media: Media | number | null | undefined): string | undefined => {
    if (media == null || typeof media === 'number' || media.url == null) {
      return undefined
    }
    return media.url !== undefined ? media.url : undefined
  }
  ...
  return (
    <div>
      <h1>Dashboard</h1>
      {logoUrl ? <img src={logoUrl} alt="Logo" /> : <div>No logo available</div>}
      <div>
        {items && items.length > 0 ? (  
          items.map(({ id, text, backgroundImage }: DashboardItem) => (
            <div key={id}>
              <h2>{text}</h2>
              <img src={getUrl(backgroundImage)} alt={text} />
            </div>
          ))
        ) : (
